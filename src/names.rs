// Generated from LandSandBoat server/base scripts/enum/job.lua and sql/zone_settings.sql.
// Checked in so CLI output does not depend on network access at runtime.

pub const JOB_NAMES: &[(u8, &str)] = &[
    (0, "NONE"),
    (1, "WAR"),
    (2, "MNK"),
    (3, "WHM"),
...
pub fn job_name(id: u8) -> Option<&'static str> {
    JOB_NAMES
        .iter()
        .find_map(|(job_id, name)| (*job_id == id).then_some(*name))
}

pub fn zone_name(id: u16) -> Option<&'static str> {
    ZONE_NAMES
        .iter()
        .find_map(|(zone_id, name)| (*zone_id == id).then_some(*name))
}
